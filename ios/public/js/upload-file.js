...
async function handleFileChange(event, id, isOldFile = false) {
  ...
  const inputFile = document.getElementById(id);
    ...
    // if (APP_STATE.urlVideo && APP_STATE.urlVideo.startsWith('blob:')) {
    //   URL.revokeObjectURL(APP_STATE.urlVideo);
    // }
    // APP_STATE.urlVideo = URL.createObjectURL(event.files[0]);

    APP_STATE.urlVideo = createManagedBlobUrl(event.files[0], APP_STATE, 'urlVideo');
