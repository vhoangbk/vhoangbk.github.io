...
/**
 * ✅ Tìm width, height, fps tối ưu dựa trên targetBitrate
 * Logic:
 * - Bước 1: Giữ nguyên width, height → thay đổi fps để đạt bitrate
 * - Bước 2: Nếu fps không hợp lệ (12-60) → giảm resolution + điều chỉnh fps
 * 
 ...
 * @param {number} originalFps - FPS ban đầu
 * @returns {Object|null} - {width, height, fps, actualBitrate} hoặc null
 */
function findOptimalConfigForBitrate(targetBitrate, format, originalWidth, originalHeight, originalFps) {
    ...
    if (!targetBitrate || targetBitrate <= 0) {
        console.error('❌ Invalid targetBitrate:', targetBitrate);
        return null;
    }

    const codecFactors = {
        27: 1.0,   // H.264 baseline
        173: 0.7,  // H.265 (30% efficient hơn)
        226: 0.6,  // AV1 (40% efficient hơn)
        167: 0.75  // VP9 (25% efficient hơn)
    };

    const codecFactor = codecFactors[codecId];
    if (!codecFactor) {
        console.error('❌ Invalid codecId:', codecId);
        return null;
    }
    ...
    // ✅ Bits per pixel (tuỳ chỉnh theo bitrate target)
    let bpp = 0.15; // Default medium quality
    if (targetBitrate < 200000) {
        bpp = 0.08; // Low bitrate → giảm bpp
    } else if (targetBitrate < 500000) {
        bpp = 0.10;
   ...
   // debugger;   
   // console.log(`🎯 Finding config: targetBitrate=${targetBitrate}bps, codec=${codecId}, original=${originalWidth}x${originalHeight}@${originalFps}fps`);

    // ✅ BƯỚC 1: Giữ nguyên width, height → tính fps
    // Formula: bitrate = width * height * fps * bpp * codecFactor
    // → fps = bitrate / (width * height * bpp * codecFactor)

    let fps = targetBitrate / (originalWidth * originalHeight * bpp * codecFactor);
    fps = Math.round(fps);

    // ✅ Nếu fps hợp lệ (12-60), return ngay
    if (fps >= minFps && fps <= maxFps) {
        const actualBitrate = originalWidth * originalHeight * fps * bpp * codecFactor;
        
        console.log(`✅ Solution (keep resolution):`, {
            width: originalWidth,
            height: originalHeight,
            fps: fps,
            targetBitrate: targetBitrate,
            actualBitrate: Math.round(actualBitrate),
            deviation: `${Math.abs((actualBitrate - targetBitrate) / targetBitrate * 100).toFixed(1)}%`
        });
    ...
    console.log(`⚠️ FPS out of range (${fps}fps), adjusting resolution...`);

    // ✅ BƯỚC 2: fps không hợp lệ → thay đổi width, height
    // Clamp fps trước
    fps = Math.max(minFps, Math.min(maxFps, fps));

    // ✅ Tính pixels cần thiết để đạt targetBitrate với fps đã clamp
    // Formula: width * height = bitrate / (fps * bpp * codecFactor)
    const requiredPixels = targetBitrate / (fps * bpp * codecFactor);

    // ✅ Tính width, height từ aspect ratio
    let width = Math.sqrt(requiredPixels * aspectRatio);
    let height = width / aspectRatio;

    // ✅ FIXED: Giữ aspect ratio khi clamp dimensions
    
    // Tính scale factors để fit trong constraints
    const maxScaleW = Math.min(maxWidth, originalWidth) / originalWidth;
    const maxScaleH = Math.min(maxHeight, originalHeight) / originalHeight;
    const minScaleW = minWidth / originalWidth;
    const minScaleH = minHeight / originalHeight;
    
    // Scale phải thỏa mãn TẤT CẢ constraints
    const maxScale = Math.min(maxScaleW, maxScaleH);
    const minScale = Math.max(minScaleW, minScaleH);
    
    // Kiểm tra nếu constraints không thể thỏa mãn
    if (minScale > maxScale) {
        console.error(`❌ Cannot satisfy constraints: minScale=${minScale.toFixed(3)} > maxScale=${maxScale.toFixed(3)}`);
        return null;
    }
    
    // Tính scale hiện tại và clamp trong range hợp lệ
    const currentScale = Math.min(width / originalWidth, height / originalHeight);
    const finalScale = Math.max(minScale, Math.min(maxScale, currentScale));
    
    // Apply scale để giữ aspect ratio
    width = Math.round(originalWidth * finalScale / 2) * 2;   // Ensure even
    height = Math.round(originalHeight * finalScale / 2) * 2; // Ensure even

    // ✅ Tính actual bitrate
    const actualBitrate = width * height * fps * bpp * codecFactor;
    
    // ✅ Verify aspect ratio preservation
    const finalAspectRatio = width / height;
    const aspectRatioError = Math.abs((finalAspectRatio - aspectRatio) / aspectRatio * 100);

    console.log(`✅ Result: ${width}x${height}@${fps}fps, bitrate=${Math.round(actualBitrate)}bps, aspectRatio=${finalAspectRatio.toFixed(3)} (${aspectRatioError.toFixed(1)}% error)`);
    ...
    return {
        ...
        fps: fps,
        actualBitrate: Math.round(actualBitrate),
        aspectRatioPreserved: aspectRatioError <= 1
    };
}
...
function findBestVideoEncoderConfigForBitrate(format, originalWidth, originalHeight, targetBitrate) {
    ...
    for (const res of uniqueResolutions) {
        ...
        for (const fps of framerates) {
            ...
            let bitsPerPixelBase = 0.15;
            
            // Giảm bpp cho targetBitrate rất thấp
            if (targetBitrate < 200000) { // < 200kbps
                bitsPerPixelBase = 0.08;  // Giảm xuống 0.08 bpp
            } else if (targetBitrate < 500000) { // < 500kbps
                bitsPerPixelBase = 0.1;   // Giảm xuống 0.1 bpp
            }
