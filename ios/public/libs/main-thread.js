...
// importScripts("utils.js?v=1763174029815");
...
const MAX_LENGTH_FILE_RAM = 1000 * 1024 * 1024; //1MB
var requestManager = {};
var saveToDiskFiles = {};
...
function add_new_encoder(ptr, length) {
    ...
    console.log('add_new_encoder config============>>>>>', JSON.stringify(config));
...
self.onmessage = async function (intent) {
    ...
    for (var i = 0; i < command.length; i++) {
        if (/^blob:|^https?:/.test(command[i])) {
            console.log('onmessage command[i]===', command[i], getUrlLength(command[i]));

            command[i] = encodeURIComponent(command[i]);
            if (command[i - 1] === '-i') {
                inputs.push(command[i]);
            }
        } else {
            command[i] = '' + command[i];
...
async function fileEvent(inputJson) {
    ...
    if (inputJson.event == 'file_open') {
        ...
        ffmpegModule.HEAPU8[inputJson.await] = 0;
    } else if (inputJson.event == 'file_read') {

        if (filename.indexOf('%') > -1 && isUrl(decodeURIComponent(filename))) {

            ffmpegModule.HEAPU8[inputJson.await] = 1;
