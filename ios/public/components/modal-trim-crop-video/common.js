...
function constrainCropBox(box, mediaWidth, mediaHeight, minSize, origWidth = null, origHeight = null) {
  ...
  if (box.w > maxW) box.w = maxW;
  if (box.h > maxH) box.h = maxH;

  box.w = roundToEven(box.w, maxW);
  box.h = roundToEven(box.h, maxH);
  ...
  if (box.x + box.w > mediaWidth) {
    ...
    box.w = roundToEven(Math.min(box.w, mediaWidth - box.x), maxW);
  }
  if (box.y + box.h > mediaHeight) {
    box.y = Math.max(0, mediaHeight - box.h);
    box.h = roundToEven(Math.min(box.h, mediaHeight - box.y), maxH);
  }
  return box;
}
...
function getCurrentCropConfig(mtcv_box, mtcv_mediaWidth, mtcv_mediaHeight, mtcv_origWidth, mtcv_origHeight, mtcv_startTime, mtcv_endTime, mtcv_flipState, mtcv_currentRatio) {
  ...
  // ✅ Làm tròn tất cả giá trị về số chẵn trước khi return
  return {
    width: roundToEven(finalWidth, mtcv_origWidth),
    height: roundToEven(finalHeight, mtcv_origHeight),
    x: roundToEven(finalX, mtcv_origWidth),
    y: roundToEven(finalY, mtcv_origHeight),
