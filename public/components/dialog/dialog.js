...
function setupDialogBlobUrlCleanup(dialog, blob_url, shouldRevoke) {
  if (shouldRevoke && blob_url && blob_url.startsWith('blob:')) {
...
function openVideoPlayerDialog() {
  ...
  const dialog = new VideoPlayerDialog({
    ...
    onClose: shouldRevoke && blob_url && blob_url.startsWith('blob:') 
      ? () => {
          try {
            setTimeout(() => {
              URL.revokeObjectURL(blob_url);
