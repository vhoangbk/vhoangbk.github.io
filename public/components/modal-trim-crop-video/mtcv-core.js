...
function handlePointerMoveInternal(ev) {
  ...
  if (mtcv_action && mtcv_action.startsWith('resize-')) {
    ...
    // =========================
    // 3. NHÁNH KHÔNG RATIO – GIỮ ANCHOR POINT KHI RESIZE
    // =========================
    
    // ✅ Lưu anchor points trước khi xử lý
    // Anchor points là các cạnh không được kéo (giữ nguyên vị trí)
    let anchorLeft = mtcv_boxStart.x;
    let anchorRight = mtcv_boxStart.x + mtcv_boxStart.w;
    let anchorTop = mtcv_boxStart.y;
    let anchorBottom = mtcv_boxStart.y + mtcv_boxStart.h;
    
    // Xác định anchor points dựa trên corner được kéo
    switch (corner) {
      case 'tl':
        // TL: giữ nguyên right và bottom
        anchorRight = mtcv_boxStart.x + mtcv_boxStart.w;
        anchorBottom = mtcv_boxStart.y + mtcv_boxStart.h;
        break;
      case 'tr':
        // TR: giữ nguyên left và bottom
        anchorLeft = mtcv_boxStart.x;
        anchorBottom = mtcv_boxStart.y + mtcv_boxStart.h;
        break;
      case 'bl':
        // BL: giữ nguyên right và top
        anchorRight = mtcv_boxStart.x + mtcv_boxStart.w;
        anchorTop = mtcv_boxStart.y;
        break;
      case 'br':
        // BR: giữ nguyên left và top
        anchorLeft = mtcv_boxStart.x;
        anchorTop = mtcv_boxStart.y;
        break;
    }
    ...
    // ✅ Điều chỉnh lại x, y để giữ anchor point
    switch (corner) {
      case 'tl':
        // Giữ nguyên right và bottom
        mtcv_box.x = anchorRight - mtcv_box.w;
        mtcv_box.y = anchorBottom - mtcv_box.h;
        break;
      case 'tr':
        // Giữ nguyên left và bottom
        mtcv_box.x = anchorLeft;
        mtcv_box.y = anchorBottom - mtcv_box.h;
        break;
      case 'bl':
        // Giữ nguyên right và top
        mtcv_box.x = anchorRight - mtcv_box.w;
        mtcv_box.y = anchorTop;
        break;
      case 'br':
        // Giữ nguyên left và top
        mtcv_box.x = anchorLeft;
        mtcv_box.y = anchorTop;
        break;
    }
    
    // ✅ Constrain width và height trước (không thay đổi position)
    ...
    // ✅ Điều chỉnh lại x, y sau khi constrain w, h để giữ anchor point
    switch (corner) {
      case 'tl':
        mtcv_box.x = anchorRight - mtcv_box.w;
        mtcv_box.y = anchorBottom - mtcv_box.h;
    ...
    // ✅ Constrain position để đảm bảo không ra ngoài bounds
    // Nhưng ưu tiên giữ anchor point nếu có thể
    if (mtcv_box.x < 0) {
      // Nếu x < 0, điều chỉnh w để giữ anchor point nếu có thể
      if (corner === 'tr' || corner === 'br') {
        // Giữ left anchor, giảm width
        const desiredRight = anchorRight;
        mtcv_box.w = Math.min(desiredRight, mtcv_mediaWidth);
        mtcv_box.w = Math.max(mtcv_minSize, roundToEven(mtcv_box.w, mtcv_mediaWidth));
        mtcv_box.x = anchorLeft;
      } else {
        // TL hoặc BL: giữ right anchor, điều chỉnh x
        mtcv_box.x = 0;
        // Điều chỉnh w để giữ right anchor nếu có thể
        const desiredRight = anchorRight;
        if (desiredRight <= mtcv_mediaWidth) {
          mtcv_box.w = desiredRight;
          mtcv_box.w = roundToEven(mtcv_box.w, mtcv_mediaWidth);
          mtcv_box.x = desiredRight - mtcv_box.w;
        } else {
          mtcv_box.w = roundToEven(mtcv_mediaWidth, mtcv_mediaWidth);
          mtcv_box.x = 0;
        }
      }
    }
    ...
    if (mtcv_box.y < 0) {
      // Nếu y < 0, điều chỉnh h để giữ anchor point nếu có thể
      if (corner === 'bl' || corner === 'br') {
        // Giữ top anchor, giảm height
        const desiredBottom = anchorBottom;
        mtcv_box.h = Math.min(desiredBottom, mtcv_mediaHeight);
        mtcv_box.h = Math.max(mtcv_minSize, roundToEven(mtcv_box.h, mtcv_mediaHeight));
        mtcv_box.y = anchorTop;
      } else {
        // TL hoặc TR: giữ bottom anchor, điều chỉnh y
        mtcv_box.y = 0;
        // Điều chỉnh h để giữ bottom anchor nếu có thể
        const desiredBottom = anchorBottom;
        if (desiredBottom <= mtcv_mediaHeight) {
          mtcv_box.h = desiredBottom;
          mtcv_box.h = roundToEven(mtcv_box.h, mtcv_mediaHeight);
          mtcv_box.y = desiredBottom - mtcv_box.h;
        } else {
          mtcv_box.h = roundToEven(mtcv_mediaHeight, mtcv_mediaHeight);
          mtcv_box.y = 0;
        }
      }
    }
    ...
    if (mtcv_box.x + mtcv_box.w > mtcv_mediaWidth) {
      // Nếu vượt quá right edge
      if (corner === 'tl' || corner === 'bl') {
        // Giữ right anchor, giảm width
        mtcv_box.w = mtcv_mediaWidth - mtcv_box.x;
        mtcv_box.w = Math.max(mtcv_minSize, roundToEven(mtcv_box.w, mtcv_mediaWidth));
        // Điều chỉnh x để giữ right anchor
        mtcv_box.x = anchorRight - mtcv_box.w;
      } else {
        // TR hoặc BR: giữ left anchor, điều chỉnh width
        mtcv_box.w = mtcv_mediaWidth - anchorLeft;
        mtcv_box.w = Math.max(mtcv_minSize, roundToEven(mtcv_box.w, mtcv_mediaWidth));
        mtcv_box.x = anchorLeft;
      }
    }
    ...
    if (mtcv_box.y + mtcv_box.h > mtcv_mediaHeight) {
      // Nếu vượt quá bottom edge
      if (corner === 'tl' || corner === 'tr') {
        // Giữ bottom anchor, giảm height
        mtcv_box.h = mtcv_mediaHeight - mtcv_box.y;
        mtcv_box.h = Math.max(mtcv_minSize, roundToEven(mtcv_box.h, mtcv_mediaHeight));
        // Điều chỉnh y để giữ bottom anchor
        mtcv_box.y = anchorBottom - mtcv_box.h;
      } else {
        // BL hoặc BR: giữ top anchor, điều chỉnh height
        mtcv_box.h = mtcv_mediaHeight - anchorTop;
        mtcv_box.h = Math.max(mtcv_minSize, roundToEven(mtcv_box.h, mtcv_mediaHeight));
        mtcv_box.y = anchorTop;
      }
    }
    
    // ✅ Final check: đảm bảo không có giá trị âm hoặc vượt quá
    if (mtcv_box.x < 0) mtcv_box.x = 0;
    if (mtcv_box.y < 0) mtcv_box.y = 0;
    if (mtcv_box.w < mtcv_minSize) mtcv_box.w = mtcv_minSize;
    if (mtcv_box.h < mtcv_minSize) mtcv_box.h = mtcv_minSize;
