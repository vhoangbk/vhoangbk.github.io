...
async function convertFileWithOptions_New(inputOptions, defaultOptions = {}) {
    ...
    if (settings.needReencode == true) {
        const isSupported = await isVideoEncoderConfigSupported(settings.format_name, settings.width, settings.height, (settings.fps < 0) ? 25 : settings.fps, settings.bitrate || 2000000);
