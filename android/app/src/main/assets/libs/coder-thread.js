    ...
    async function input_looper() {
        while (true) {
            ...
            for (var i = 0; i < keys.length; i++) {
                ...
                if (coder.is_encoder) {
                    const fps = coder.config.framerate_den > 0 ? coder.config.framerate_num / coder.config.framerate_den : 30;
                    const gopSize = Math.min(fps * 2, is_safari == true ? 30 : 30);
                    ...
                    if (is_keyframe && coder.settings?.target_size && coder.name !== ex_encoder_key
                        && out_duration >= Math.max(1, Math.min(coder.settings.duration / 10, 10))) {
                       ...
                       // console.log(`Adjusted bitrate to ${coder.config.bit_rate} bps, output total bitrate: ${bitrate_output_total.toFixed(0)} bps`);
                    }
