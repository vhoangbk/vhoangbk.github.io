...
function updateResolutionOptions(restore = false) {
  ...
  if (!codecData?.Main?.supported_resolution) {
    // Gen None giống như khởi tạo
  ...
  filteredResolutions.forEach(([w, h]) => {
    const res = w > h ? `${w}x${h}` : `${h}x${w}`;
    items.push({ value: res, text: res });
  });
...
function initCustomSelects() {
  ...
  document.querySelectorAll(".custom-select").forEach(select => {
    ...
    optionsBox.addEventListener("click", (e) => {
      e.stopPropagation();  // <-- FIX LỖI OVERLAY KHÔNG ĐÓNG
...
function fillCustomSelect(elOrId, items, restore = false, isInit = false) {
  ...
  if (isMobile) {
    ...
    landscapeGroup.className = "custom-options-group active";
...
function renderTargetSize() {
  ...
  highlighSelectedOption(taregrSizeSelect, "");
}
